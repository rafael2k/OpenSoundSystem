		      ...
		      unsigned int pci_subdevice, int group_type)
{
...

  {
    ...
    /*
     * Try reading the vendor id first, exactly like ALSA does (works
    ...
    if (nretry == 3)
      {
	/* Codec not answering: reset it and power up (takes ~250 ms) */
	corb_write (mixer, cad, 0, 0, SET_CODEC_RESET, 0);
	oss_udelay (250000);
	corb_write (mixer, cad, 0, 0, SET_POWER_STATE, 0);
	oss_udelay (10000);
